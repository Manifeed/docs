...
```mermaid
flowchart LR
    admin["Frontend admin"] --> route["POST /rss/ingest"]
    ...
    tasks --> response[["200 RssScrapeJobQueuedRead"]]
    lock -->|occupe| e409[["409 already running"]]
```
...
- Securite : `Session admin`.
- Inputs :
  - Query optionnelle `feed_ids[]` de type `PositiveInt`.
- Output :
  - `200` `RssScrapeJobQueuedRead`.
- Erreurs :
...
- Processus :
  ...
  3. charge les feeds demandes ou tous les feeds `enabled` dont la compagnie est active ;
  4. construit des batches de 20 feeds ;
  5. cree un `worker_jobs` `rss_scrape` ;
  6. cree un `worker_tasks` par batch avec payload JSON ;
  7. commit ;
  8. si aucun feed n'est eligibile, cree un job `completed` sans tasks.
